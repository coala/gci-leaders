...
const CHAT_IMAGES = {
  ...
  DISCOURSE: 'images/logos/discourse.png',
...
async function fetchOrgsWithData() {
  ...
  const fetchingMailingList = orgs.map(org => {
    return chattie(org.mailing_list)
  })
  ...
  const orgMailingList = await Promise.all(fetchingMailingList)
  ...
  const fetchingAll = orgs.map(async (org, index) => {
    ...
    return Object.assign(org, {
      ...
      chat: {
        ...
        platform: orgChats[index].platform ? orgChats[index].platform
          : chattie.CHAT[orgChats[index].type],
        image: orgChats[index].image ? orgChats[index].image
          : CHAT_IMAGES[chattie.CHAT[orgChats[index].type]],
      },
      mailing_list: {
        url: org.mailing_list,
        image: CHAT_IMAGES[chattie.CHAT[orgMailingList[index].type]]
          ? CHAT_IMAGES[chattie.CHAT[orgMailingList[index].type]] 
          : 'mail.png'
      },
