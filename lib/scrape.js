...
async function fetchProgram() {
  const res = await fetch(`${GCI_API_BASE}/program/${year}/`)
  if (res.status == 200) {
    return await res.json()
  }

  year -= 1
  const lastYearApi = `${GCI_API_BASE}/program/${year}/`
  return await (await fetch(lastYearApi)).json()
}

async function fetchOrgs() {
  const res = await fetch(
    `${GCI_API_BASE}/program/${year}/organization/?status=2`
  )
  ...
  year -= 1
  const lastYearApi = `${GCI_API_BASE}/program/${year}/`
  const { results } = await (await fetch(lastYearApi)).json()
